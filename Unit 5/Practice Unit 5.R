...
library(countrycode)
countrycode("Albania", 'country.name', 'iso2c') # syntax- "<Name of the Country>", '<Variable Name>', '<output>'
...
GDPPC = WDI(indicator="NY.GDP.PCAP.KD", start=2019, end=2019)
head(GDPPC)

library(WHO) #### WHO package is not available in R 4.0.0

########## Install Dev tools first ############
library(devtools)

######### Install WHO package from Github instead ##############
######### https://rdrr.io/cran/WHO/f/vignettes/who_vignette.Rmd ############
...
codes <- get_codes()#The get_codes function returns a data frame with series codes and descriptions for all available series
str(codes)
dim(codes)
names(codes)
...
ThePlot = GDPPC_LE %>% 
  ggplot(aes(NY.GDP.PCAP.KD,value,color = Country)) + geom_point() + xlab("GDP Per Capita") +   ylab("Life Expectancy") + ggtitle("Life Expectancy v. GDP Per Capita for 2019")
ggplotly(ThePlot)
...
str_view(string1, "^Don't") # ^ is an Anchor  to look for at the begining of the string
str_view(string1, "^interfere")
str_view(string3, "^T")
str_view(string6, "^\"Some") #Why Nothing ... we will come back to this.
str_view(string6, "Jobs$") # $ wildcard for look for at the end of the string
...
str_view_all(df$quotes,"[^abc]") # ^ inside [] means NOT. So this is looking for NOT a, b or c
